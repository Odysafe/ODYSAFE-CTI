...
from .pinned_sources import clone_pinned_repo
...
class GitHubRepoManager:
    ...
    def check_manual_repo(self, custom_path: Optional[Path] = None) -> bool:
        """Check if a repository exists at a custom path or can be linked.
        
        Args:
            custom_path: Optional custom path to check for repository
            
        Returns:
            bool: True if repository exists and is valid
        """
        # Check primary location first
        if self.repo_exists():
            logger.info(f"Repository exists at standard location: {self.repo_dir}")
            return True
        
        # Check custom path if provided
        if custom_path and custom_path.exists():
            if (custom_path / ".git").exists():
                logger.info(f"Repository found at custom path: {custom_path}")
                return True
        
        # Check common alternative locations
        alternative_paths = [
            Path.home() / "deepdarkCTI",
            Path.home() / "deepdarkCTI-main",
            Path("/tmp/deepdarkCTI"),
        ]
        
        for alt_path in alternative_paths:
            if alt_path.exists() and (alt_path / ".git").exists():
                logger.info(f"Repository found at alternative location: {alt_path}")
                return True
        
        logger.info("No repository found at standard or alternative locations")
        return False
    
    def use_local_repo(self, source_path: Path) -> bool:
        """Use a local repository by copying or symlinking it.
        
        Args:
            source_path: Path to the local repository
            
        Returns:
            bool: True if successfully set up
        """
        try:
            ...
            if not (source_path / ".git").exists():
                logger.error(f"Source path is not a git repository: {source_path}")
                return False
            
            # Remove existing repo if present
            if self.repo_dir.exists():
                logger.info(f"Removing existing repository at {self.repo_dir}")
                shutil.rmtree(self.repo_dir, ignore_errors=True)
            
            # Create parent directory
            self.repo_dir.parent.mkdir(parents=True, exist_ok=True)
            
            # Try to create symlink first (preferred for user mode)
            try:
                self.repo_dir.symlink_to(source_path.resolve())
                logger.info(f"Created symlink from {source_path} to {self.repo_dir}")
                return True
            except (OSError, NotImplementedError):
                # Symlink not supported, fall back to copy
                logger.info(f"Symlink not supported, copying repository...")
                shutil.copytree(source_path, self.repo_dir)
                logger.info(f"Repository copied from {source_path} to {self.repo_dir}")
                return True
    ...
    def download_repo(self) -> bool:
        ...
        try:
            ...
            with download_lock("deepdarkcti", timeout=300):
                ...
                logger.info(f"Cloning pinned DeepDarkCTI snapshot to {temp_clone_dir}...")
                if not clone_pinned_repo("deepdarkcti", temp_clone_dir, timeout=120):
                    return False
