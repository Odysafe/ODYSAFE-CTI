...
def get_iocs_for_export(db, data: Dict) -> Tuple[List[Dict], Optional[str]]:
    ...
    # Tag-based export
    if tag_ids:
        iocs = db.get_iocs_by_tag_ids([int(t) for t in tag_ids])
        if ioc_types:
            iocs = [ioc for ioc in iocs if ioc.get('ioc_type') in ioc_types]
        # Dedup and return early
        seen_iocs = {}
        unique_iocs = []
        for ioc in iocs:
            key = (ioc.get('ioc_type','').lower(), ioc.get('ioc_value','').lower())
...
def _create_export_file_path(output_folder: Path, filename_prefix: str, extension: str) -> Path:
    """Helper function to create standardized export file paths"""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = output_folder / "iocs" / f"{filename_prefix}_{timestamp}.{extension}"
...
def export_txt(db, data: Dict, output_folder: Path) -> Path:
    ...
    output_file = _create_export_file_path(output_folder, "export", "txt")
...
def export_txt_simple(db, data: Dict, output_folder: Path) -> Path:
    ...
    iocs, _ = get_iocs_for_export(db, data)
    output_file = _create_export_file_path(output_folder, "export_simple", "txt")
...
def export_json(db, data: Dict, output_folder: Path) -> Path:
    ...
    iocs, _ = get_iocs_for_export(db, data)
    output_file = _create_export_file_path(output_folder, "export", "json")
...
def export_json_simple(db, data: Dict, output_folder: Path) -> Path:
    ...
    iocs, _ = get_iocs_for_export(db, data)
    output_file = _create_export_file_path(output_folder, "export_simple", "json")
