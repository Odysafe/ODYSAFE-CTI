...
from modules.mitre_stix_parser import (
    MitreStixParser, mitre_json_exists, get_mitre_json_path
)
...
from routes_log_analyzer import log_analyzer_bp
...
def _extract_iocs_background(source_id: int, extraction_func, extraction_args: tuple, 
    ...
    try:
        ...
        # Index source content in local CTI memory (ZettelForge, no LLM)
        try:
            from modules import zettelforge_bridge as zf
            raw_text = None
            if getattr(extraction_func, '__name__', '') == 'extract_from_text' and extraction_args:
                raw_text = extraction_args[0]
            elif extraction_args:
                raw_text = zf.extract_text_from_file(str(extraction_args[0]))
            all_iocs = [
                {'ioc_type': _normalize_ioc_type(t), 'ioc_value': v}
                for t, v, _, _ in results
            ]
            zf.remember_source_import(
                source_id, source_info, all_iocs or iocs_list, raw_text=raw_text
            )
        except Exception as mem_err:
            logger.debug('CTI memory indexing skipped: %s', mem_err)
...
def iocs_list():
    ...
    try:
        ...
        search = request.args.get('search', '').strip() or request.args.get('value', '').strip()
...
def api_ioc_notes(ioc_id):
    ...
    try:
        ...
        try:
            ...
            logger.debug('CTI memory IOC note skipped: %s', mem_err)
...
def api_cleanup_all_data():
    ...
    try:
        ...
        results = {
            ...
            'groups_deleted': 0,
            'incidents_deleted': 0,
            'investigation_sessions_deleted': 0,
        ...
        # 4. Delete all incidents and incident events from database
        try:
            with db.connection() as conn:
                # Get incident count before deletion
                cursor = conn.execute("SELECT COUNT(*) as count FROM incidents")
                results['incidents_deleted'] = cursor.fetchone()['count']
                
                # Delete incident events first (foreign key constraint)
                conn.execute("DELETE FROM incident_events")
                conn.execute("DELETE FROM incident_ioc_links")
                # Delete incidents
                conn.execute("DELETE FROM incidents")
                conn.commit()
        except Exception as e:
            logger.warning(f"Error deleting incidents: {e}")
        
        # 5. Delete relational analysis data and core database content
        with db.connection() as conn:
            cursor = conn.execute("SELECT COUNT(*) as count FROM investigation_sessions")
            results['investigation_sessions_deleted'] = cursor.fetchone()['count']
        ...
        message = (
            ...
            f"{results['incidents_deleted']} incidents, "
            f"{results['investigation_sessions_deleted']} investigation sessions, "
...
def api_settings_import_zip():
    ...
    try:
        ...
        if stats.get('investigation_sessions_imported'):
            parts.append(f"{stats['investigation_sessions_imported']} investigation sessions")
        if stats.get('incidents_imported'):
            parts.append(f"{stats['incidents_imported']} log incidents")
...
def api_mitre_group_ioc_overlap(group_id):
    ...
    try:
        ...
        matched_ids = set(db.get_ttp_ioc_values_for_group(tech_ids))
...
def api_global_search():
    """Global cross-entity search: IOCs, Sources, MITRE."""
    try:
        q = request.args.get('q', '').strip()
        if not q or len(q) < 2:
            return api_success({'iocs': [], 'sources': [], 'mitre': [], 'query': q})

        like = f'%{q.replace("%", "\\%").replace("_", "\\_")}%'
        results = {}

        with db.connection() as conn:
            # IOCs: exact match ranked first (score=0), then LIKE (score=1)
            # Group by value+type so duplicates across sources are merged;
            # pick the id of the canonical (first-created) record for linking.
            rows = conn.execute("""
            ...
            # Sources: exact name match first, then LIKE on name or context
            rows = conn.execute("""
        ...
        # MITRE: use existing parser search
        mitre_results = []
        try:
            if mitre_json_exists():
                parser = MitreStixParser(get_mitre_json_path())
                if parser.load():
                    sr = parser.search(q)
                    for t in (sr.get('techniques') or [])[:3]:
                        mitre_results.append({'kind': 'technique', 'id': t.get('id',''), 'name': t.get('name',''), 'tactic': t.get('tactic','')})
                    for g in (sr.get('groups') or [])[:2]:
                        mitre_results.append({'kind': 'group', 'id': g.get('id',''), 'name': g.get('name','')})
        except Exception:
            pass
        results['mitre'] = mitre_results

        # CTI Memory (ZettelForge semantic recall, no LLM)
        memory_results = []
        try:
            from modules import zettelforge_bridge as zf
            if zf.is_available():
                memory_results = zf.recall_query(q, k=5)[:5]
        except Exception:
            pass
