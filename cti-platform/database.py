...
class Database:
    ...
    def init_database(self):
        ...
        # Table incidents (log analysis incidents)
        ody.execute("""
        ...
        """)
        ody.execute("CREATE INDEX IF NOT EXISTS idx_incidents_created ON incidents(created_at)")
        ody.execute("CREATE INDEX IF NOT EXISTS idx_incidents_status ON incidents(status)")

        # Table incident_events (individual log events mapped to MITRE)
        ody.execute("""
        ...
        """)
        ody.execute("CREATE INDEX IF NOT EXISTS idx_incident_events_incident ON incident_events(incident_id)")
        ody.execute("CREATE INDEX IF NOT EXISTS idx_incident_events_technique ON incident_events(mitre_technique_id)")
        ody.execute("CREATE INDEX IF NOT EXISTS idx_incident_events_tactic ON incident_events(mitre_tactic)")

        # Table incident_ioc_links (link incident events to IOCs in database)
        ody.execute("""
            CREATE TABLE IF NOT EXISTS incident_ioc_links (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                incident_id TEXT NOT NULL,
                ...
                FOREIGN KEY (ioc_id) REFERENCES iocs(id) ON DELETE CASCADE,
                UNIQUE(incident_id, event_id, ioc_id)
            )
        """)
        ody.execute("CREATE INDEX IF NOT EXISTS idx_incident_ioc_links_incident ON incident_ioc_links(incident_id)")
        ody.execute("CREATE INDEX IF NOT EXISTS idx_incident_ioc_links_ioc ON incident_ioc_links(ioc_id)")

        # Investigation sessions (lightweight workspace for an analyst session)
        ody.execute("""
        ...
        """)
        ody.execute("CREATE INDEX IF NOT EXISTS idx_investigation_sessions_status ON investigation_sessions(status)")
    ...
    def get_ttp_ioc_values_for_group(self, technique_ids: List[str]) -> List[str]:
        """Returns distinct TTP IOC values (uppercased) that match any of the given technique IDs."""
        if not technique_ids:
            return []
        placeholders = ','.join('?' * len(technique_ids))
        with self.connection() as conn:
            rows = conn.execute(
                f"SELECT DISTINCT UPPER(ioc_value) FROM iocs WHERE is_deleted=0 AND ioc_type='ttp' AND UPPER(ioc_value) IN ({placeholders})",
                [t.upper() for t in technique_ids]
            ).fetchall()
    ...
    def create_mitre_tactic(self, tactic_id: str, name: str, description: str = None,
                            url: str = None, shortname: str = None) -> bool:
        """Creates a MITRE ATT&CK tactic"""
        with self.connection() as conn:
            ody = conn.cursor()
            try:
                ody.execute("""
                    INSERT OR REPLACE INTO mitre_tactics (id, name, description, url, shortname)
                    VALUES (?, ?, ?, ?, ?)
                """, (tactic_id, name, description, url, shortname))
                conn.commit()
                return True
            except Exception:
                return False

    def create_mitre_technique(self, technique_id: str, name: str, description: str = None,
                               url: str = None, is_subtechnique: bool = False,
                               parent_id: str = None, deprecated: bool = False) -> bool:
        """Creates a MITRE ATT&CK technique"""
        with self.connection() as conn:
            ody = conn.cursor()
            try:
                ody.execute("""
                    INSERT OR REPLACE INTO mitre_techniques
                    (id, name, description, url, is_subtechnique, parent_id, deprecated)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (technique_id, name, description, url,
                      1 if is_subtechnique else 0, parent_id, 1 if deprecated else 0))
                conn.commit()
                return True
            except Exception:
                return False

    def link_technique_to_tactic(self, technique_id: str, tactic_id: str) -> bool:
        """Links a technique to a tactic"""
        with self.connection() as conn:
            ody = conn.cursor()
            try:
                ody.execute("""
                    INSERT OR IGNORE INTO mitre_technique_tactics (technique_id, tactic_id)
                    VALUES (?, ?)
    ...
    def create_mitre_group(self, group_id: str, name: str, aliases: str = None,
                           description: str = None, url: str = None) -> bool:
        """Creates a MITRE ATT&CK group (APT)"""
        with self.connection() as conn:
            ody = conn.cursor()
            try:
                ody.execute("""
                    INSERT OR REPLACE INTO mitre_groups (id, name, aliases, description, url)
                    VALUES (?, ?, ?, ?, ?)
                """, (group_id, name, aliases, description, url))
                conn.commit()
                return True
            except Exception:
                return False

    def create_mitre_software(self, software_id: str, name: str, s_type: str = None,
                            description: str = None, url: str = None) -> bool:
        """Creates a MITRE ATT&CK software (malware/tool)"""
        with self.connection() as conn:
            ody = conn.cursor()
            try:
                ody.execute("""
                    INSERT OR REPLACE INTO mitre_software (id, name, type, description, url)
                    VALUES (?, ?, ?, ?, ?)
                """, (software_id, name, s_type, description, url))
                conn.commit()
                return True
            except Exception:
                return False

    def create_mitre_group_use(self, use_id: str, group_id: str, technique_id: str,
                               software_id: str = None, use_description: str = None) -> bool:
        """Records a group's use of a technique"""
        with self.connection() as conn:
            ody = conn.cursor()
            try:
                ody.execute("""
                    INSERT OR REPLACE INTO mitre_group_uses
                    (id, group_id, technique_id, software_id, use_description)
                    VALUES (?, ?, ?, ?, ?)
                """, (use_id, group_id, technique_id, software_id, use_description))
                conn.commit()
                return True
            except Exception:
                return False

    def create_mitre_mitigation(self, mitigation_id: str, name: str,
                                description: str = None, url: str = None) -> bool:
        """Creates a MITRE ATT&CK mitigation"""
        with self.connection() as conn:
            ody = conn.cursor()
            try:
                ody.execute("""
                    INSERT OR REPLACE INTO mitre_mitigations (id, name, description, url)
                    VALUES (?, ?, ?, ?)
                """, (mitigation_id, name, description, url))
                conn.commit()
                return True
            except Exception:
                return False

    def link_technique_to_mitigation(self, technique_id: str, mitigation_id: str) -> bool:
        """Links a technique to a mitigation"""
        with self.connection() as conn:
            ody = conn.cursor()
            try:
                ody.execute("""
                    INSERT OR IGNORE INTO mitre_technique_mitigations (technique_id, mitigation_id)
                    VALUES (?, ?)
    ...
    def get_mitre_tactics(self) -> List[Dict]:
        """Gets all MITRE ATT&CK tactics"""
        with self.connection() as conn:
            ody = conn.cursor()
            rows = ody.execute("""
                SELECT id, name, description, url, shortname
                FROM mitre_tactics
                ORDER BY id
            """).fetchall()
            return [dict(row) for row in rows]

    def get_mitre_techniques(self, tactic_id: str = None, search: str = None) -> List[Dict]:
        """Gets MITRE ATT&CK techniques, optionally filtered by tactic or search"""
        with self.connection() as conn:
            ody = conn.cursor()
            query = """
                SELECT t.id, t.name, t.description, t.url, t.is_subtechnique,
                       t.parent_id, t.deprecated, GROUP_CONCAT(tt.tactic_id) as tactics
            ...
            params = []
            if tactic_id:
                query += " AND tt.tactic_id = ?"
                params.append(tactic_id)
            if search:
                query += " AND (t.id LIKE ? OR t.name LIKE ?)"
                params.extend([f"%{search}%", f"%{search}%"])
            query += " GROUP BY t.id ORDER BY t.id"
            rows = ody.execute(query, params).fetchall()
            return [dict(row) for row in rows]

    def get_mitre_technique_by_id(self, technique_id: str) -> Optional[Dict]:
        """Gets a single technique with its tactics and mitigations"""
        with self.connection() as conn:
            ody = conn.cursor()
            row = ody.execute("""
                SELECT id, name, description, url, is_subtechnique, parent_id, deprecated
            ...
            # Get tactics
            tactics = ody.execute("""
                SELECT t.id, t.name FROM mitre_tactics t
                JOIN mitre_technique_tactics tt ON t.id = tt.tactic_id
                WHERE tt.technique_id = ?
            """, (technique_id,)).fetchall()
            result['tactics'] = [dict(t) for t in tactics]
            # Get mitigations
            mitigations = ody.execute("""
                SELECT m.id, m.name FROM mitre_mitigations m
                JOIN mitre_technique_mitigations tm ON m.id = tm.mitigation_id
                WHERE tm.technique_id = ?
            """, (technique_id,)).fetchall()
            result['mitigations'] = [dict(m) for m in mitigations]
            # Get groups using this technique
            groups = ody.execute("""
                SELECT g.id, g.name FROM mitre_groups g
                JOIN mitre_group_uses gu ON g.id = gu.group_id
                WHERE gu.technique_id = ?
            """, (technique_id,)).fetchall()
    ...
    def get_mitre_software(self, s_type: str = None) -> List[Dict]:
        """Gets MITRE ATT&CK software"""
        with self.connection() as conn:
            ody = conn.cursor()
            query = "SELECT id, name, type, description, url FROM mitre_software"
            params = []
            if s_type:
                query += " WHERE type = ?"
    ...
    def clear_mitre_data(self) -> bool:
        """Clears all MITRE ATT&CK data (for reload)"""
        with self.connection() as conn:
            ody = conn.cursor()
            try:
                ody.execute("DELETE FROM mitre_technique_mitigations")
                ody.execute("DELETE FROM mitre_technique_tactics")
                ody.execute("DELETE FROM mitre_group_uses")
                ody.execute("DELETE FROM mitre_mitigations")
                ody.execute("DELETE FROM mitre_software")
                ody.execute("DELETE FROM mitre_groups")
                ody.execute("DELETE FROM mitre_techniques")
                ody.execute("DELETE FROM mitre_tactics")
                conn.commit()
                return True
            except Exception:
                return False

    def count_mitre_data(self) -> Dict:
        """Returns counts of MITRE data for quick status check"""
        with self.connection() as conn:
            ody = conn.cursor()
            counts = {}
            for table in ['mitre_tactics', 'mitre_techniques', 'mitre_groups',
                          'mitre_software', 'mitre_mitigations']:
                row = ody.execute(f"SELECT COUNT(*) FROM {table}").fetchone()
                counts[table] = row[0] if row else 0
            return counts
    
    def fix_invalid_mitre_tactics(self) -> Dict:
        """Fix invalid MITRE ATT&CK tactics (e.g., TA0112 Defense Impairment doesn't exist)"""
        # Valid Enterprise ATT&CK tactics (14 tactics)
        valid_tactics = [
        ...
        fixed = {'removed': [], 'remapped': 0}
        
        with self.connection() as conn:
            ody = conn.cursor()
            
            # Find invalid tactics
            ody.execute("SELECT id, name FROM mitre_tactics WHERE id NOT IN ({})".format(
                ','.join(['?'] * len(valid_tactics))
            ), valid_tactics)
            invalid_tactics = ody.fetchall()
            
            for tactic_id, tactic_name in invalid_tactics:
                # Remove invalid tactic from technique mappings
                ody.execute("DELETE FROM mitre_technique_tactics WHERE tactic_id = ?", (tactic_id,))
                # Remove the invalid tactic itself
                ody.execute("DELETE FROM mitre_tactics WHERE id = ?", (tactic_id,))
                fixed['removed'].append({'id': tactic_id, 'name': tactic_name})
            
            # Fix TA0112 specifically - if it exists, remap to TA0005 (Defense Evasion)
            ody.execute("SELECT COUNT(*) FROM mitre_tactics WHERE id = 'TA0112'")
            if ody.fetchone()[0] > 0:
                # Update any technique mappings from TA0112 to TA0005
                ody.execute("""
                    UPDATE OR IGNORE mitre_technique_tactics 
                    SET tactic_id = 'TA0005' 
                    WHERE tactic_id = 'TA0112'
                """)
                fixed['remapped'] = ody.rowcount
                # Delete TA0112
                ody.execute("DELETE FROM mitre_tactics WHERE id = 'TA0112'")
                fixed['removed'].append({'id': 'TA0112', 'name': 'Defense Impairment'})
    ...
    def get_ioc_expiry_rules(self) -> dict:
        """Get default expiry rules by IOC type"""
        return {
            'ip4': 90,      # 90 days for IPv4
            'ip6': 90,      # 90 days for IPv6
            'fqdn': 180,    # 180 days for domains
            'url': 90,      # 90 days for URLs
            'md5': 365,     # 1 year for MD5 hashes
            'sha1': 365,    # 1 year for SHA1 hashes
            'sha256': 365,  # 1 year for SHA256 hashes
            'email': 180,   # 180 days for emails
            'bitcoin': 365, # 1 year for crypto addresses
            'ethereum': 365, # 1 year for crypto addresses
            'cve': 730,     # 2 years for CVEs
            'ttp': 365,     # 1 year for TTPs
            'onionaddress': 365, # 1 year for Tor addresses
            'packagename': 365, # 1 year for packages
            'webmoney': 365, # 1 year for crypto
            'twitter': 180, # 180 days for Twitter
            'github': 365,  # 1 year for GitHub
            'instagram': 180, # 180 days for Instagram
            'linkedin': 365, # 1 year for LinkedIn
            'facebook': 180, # 180 days for Facebook
            'youtube': 365, # 1 year for YouTube
            'telegram': 365, # 1 year for Telegram
            'pinterest': 180, # 180 days for Pinterest
        }
    
    def set_ioc_expiry_date(self, ioc_id: int, ioc_type: str = None, custom_days: int = None) -> bool:
        """Set expiry date for an IOC based on type or custom days"""
        with self.connection() as conn:
            ody = conn.cursor()
            
            if custom_days:
                expiry_date = f"datetime('now', '+{custom_days} days')"
            else:
                if not ioc_type:
                    # Get IOC type from database
                    ody.execute("SELECT ioc_type FROM iocs WHERE id = ?", (ioc_id,))
                    result = ody.fetchone()
                    if not result:
                        return False
                    ioc_type = result['ioc_type']
                
                rules = self.get_ioc_expiry_rules()
                days = rules.get(ioc_type, 90)  # Default 90 days
                expiry_date = f"datetime('now', '+{days} days')"
            
            ody.execute(f"""
                UPDATE iocs 
                SET expiry_date = {expiry_date}, lifecycle_status = 'Active'
                WHERE id = ?
            """, (ioc_id,))
    ...
    def get_expired_iocs(self) -> list:
        ...
        with self.connection() as conn:
            ...
            ody.execute("""
                ...
                AND i.lifecycle_status != 'Expired'
                AND i.is_deleted = 0
                ORDER BY i.expiry_date ASC
            """)
            return [dict(row) for row in ody.fetchall()]
    
    def expire_iocs(self, ioc_ids: list = None) -> int:
        """Mark IOCs as expired (all expired or specific IDs)"""
        with self.connection() as conn:
            ody = conn.cursor()
            
            if ioc_ids:
                # Expire specific IOCs
    ...
    def update_ioc_lifecycle(self, ioc_id: int, status: str, expiry_days: int = None) -> bool:
        """Update IOC lifecycle status and optionally set expiry"""
        valid_statuses = ['Active', 'Deprecated', 'Expired', 'Retired', 'Under Review']
        ...
        with self.connection() as conn:
            ...
            if expiry_days:
                expiry_date = f"datetime('now', '+{expiry_days} days')"
                ody.execute(f"""
                    UPDATE iocs 
                    SET lifecycle_status = ?, expiry_date = {expiry_date}
                    WHERE id = ?
                """, (status, ioc_id))
            else:
                ody.execute("""
    ...
    def get_ioc_lifecycle_stats(self) -> dict:
        """Get statistics about IOC lifecycle status"""
        with self.connection() as conn:
            ody = conn.cursor()
            ody.execute("""
                SELECT 
                    lifecycle_status,
                    COUNT(*) as count,
                    COUNT(CASE WHEN expiry_date < datetime('now') THEN 1 END) as expired_count
                FROM iocs 
                WHERE is_deleted = 0
    ...
    def cleanup_expired_iocs(self, delete_after_days: int = 30) -> int:
        """Permanently delete IOCs that have been expired for specified days"""
        with self.connection() as conn:
            ody = conn.cursor()
            
            cutoff_date = f"datetime('now', '-{delete_after_days} days')"
            
            ody.execute(f"""
                DELETE FROM iocs 
                WHERE lifecycle_status = 'Expired'
                AND expiry_date < {cutoff_date}
            """)
            
            conn.commit()
            return ody.rowcount

    def get_active_investigation_session(self) -> Optional[Dict]:
        """Return the current active investigation session, if any."""
        with self.connection() as conn:
            row = conn.execute("""
    ...
    def save_investigation_session(
        self,
        name: str,
        notes: str = '',
        ...
        """Create or update an investigation session. Only one active session at a time."""
        with self.connection() as conn:
            ody = conn.cursor()
            if session_id:
                ody.execute("""
                    UPDATE investigation_sessions
                    SET name = ?, notes = ?, group_id = ?, updated_at = CURRENT_TIMESTAMP
                    WHERE id = ?
                """, (name.strip(), notes or '', group_id, session_id))
                conn.commit()
                return session_id
            ...
            """)
            ody.execute("""
                INSERT INTO investigation_sessions (name, notes, group_id, status)
                VALUES (?, ?, ?, 'active')
            """, (name.strip(), notes or '', group_id))
            conn.commit()
            return ody.lastrowid

    def close_investigation_session(self, session_id: int) -> bool:
        with self.connection() as conn:
            ody = conn.cursor()
            ody.execute("""
                UPDATE investigation_sessions
    ...
    def get_investigation_session_stats(self, group_id: Optional[int] = None) -> Dict:
        """Counts for session dashboard: IOCs, TTP links, log incidents."""
        stats = {
            'ioc_count': 0,
            'true_positive_count': 0,
            'ttp_link_count': 0,
            'log_incident_count': 0,
            'log_technique_count': 0,
        }
        with self.connection() as conn:
            if group_id:
                row = conn.execute("""
                    SELECT COUNT(DISTINCT i.id) AS ioc_count,
                           SUM(CASE WHEN i.validation_status = 'True Positive' THEN 1 ELSE 0 END) AS tp_count
                    FROM iocs i
                    JOIN ioc_groups ig ON ig.ioc_id = i.id
                    WHERE ig.group_id = ? AND i.is_deleted = 0
                """, (group_id,)).fetchone()
                if row:
                    stats['ioc_count'] = row['ioc_count'] or 0
                    stats['true_positive_count'] = row['tp_count'] or 0

                row = conn.execute("""
                    SELECT COUNT(DISTINCT itt.technique_id) AS ttp_count
                    FROM ioc_ttp_links itt
                    JOIN ioc_groups ig ON ig.ioc_id = itt.ioc_id
                    JOIN iocs i ON i.id = itt.ioc_id
                    WHERE ig.group_id = ? AND i.is_deleted = 0
                """, (group_id,)).fetchone()
                if row:
                    stats['ttp_link_count'] = row['ttp_count'] or 0
            else:
                row = conn.execute("""
                    SELECT COUNT(*) AS ioc_count,
                           SUM(CASE WHEN validation_status = 'True Positive' THEN 1 ELSE 0 END) AS tp_count
                    FROM iocs WHERE is_deleted = 0
                """).fetchone()
                if row:
                    stats['ioc_count'] = row['ioc_count'] or 0
                    stats['true_positive_count'] = row['tp_count'] or 0
                ...
                row = conn.execute("""
                    SELECT COUNT(DISTINCT technique_id) AS ttp_count
                    FROM ioc_ttp_links
                """).fetchone()
                if row:
                    stats['ttp_link_count'] = row['ttp_count'] or 0
            ...
            stats['log_incident_count'] = row['c'] if row else 0

            row = conn.execute("""
                SELECT COUNT(DISTINCT mitre_technique_id) AS c
                FROM incident_events
                WHERE mitre_technique_id IS NOT NULL AND mitre_technique_id != ''
            """).fetchone()
            stats['log_technique_count'] = row['c'] if row else 0

        return stats

    def get_ttp_coverage(self, group_id: Optional[int] = None) -> Dict:
        """Aggregate MITRE techniques from IOC links and log analyzer events."""
        from_iocs = []
        from_logs = []
        ...
        with self.connection() as conn:
            if group_id:
                rows = conn.execute("""
                    SELECT itt.technique_id,
                           COUNT(DISTINCT itt.ioc_id) AS ioc_count
                    FROM ioc_ttp_links itt
                    JOIN ioc_groups ig ON ig.ioc_id = itt.ioc_id
                    JOIN iocs i ON i.id = itt.ioc_id
                    WHERE ig.group_id = ? AND i.is_deleted = 0
                    GROUP BY itt.technique_id
                    ORDER BY ioc_count DESC, itt.technique_id
                """, (group_id,)).fetchall()
            else:
                rows = conn.execute("""
                    SELECT itt.technique_id,
            ...
            rows = conn.execute("""
                SELECT mitre_technique_id AS technique_id,
                       MAX(mitre_technique_name) AS technique_name,
                       MAX(mitre_tactic) AS tactic,
                       COUNT(*) AS event_count,
                       COUNT(DISTINCT incident_id) AS incident_count
                FROM incident_events
                WHERE mitre_technique_id IS NOT NULL AND mitre_technique_id != ''
                GROUP BY mitre_technique_id
                ORDER BY event_count DESC, mitre_technique_id
            """).fetchall()
        ...
        return {'from_iocs': from_iocs, 'from_logs': from_logs}
