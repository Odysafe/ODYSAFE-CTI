...
EOF
    echo -e "${NC}"
    printf "%s\n" "             ${CYAN}${BOLD}Cyber Threat Intelligence Platform${NC}"
    printf "%s\n" "             ${DIM}https://github.com/odysafe/cti-platform${NC}"
    echo ""
    hline 64 "$H" "$DIM"
...
# ── Step 6 – MITRE ATT&CK data ────────────────────────────────────────────────

prompt_mitre_download() {
    step "MITRE ATT&CK data"
    ...
    echo ""
    printf "  ${CYAN}?${NC}  ${BOLD}Download MITRE ATT&CK enterprise-attack.json now?${NC}\n"
    printf "  ${YELLOW}⚠${NC}  This file is voluminous (~50 MB).\n"
    printf "  ${DIM}Press Enter to accept the recommended download.${NC}\n\n"

    if prompt_yes_no "Download MITRE ATT&CK now?" "Y"; then
        ...
        if download_mitre_json "$mitre_file"; then
            ...
            log_warn "Download failed — you can download later from Analysis → MITRE ATT&CK"
            log_dim "The web UI uses Python urllib (curl is not required)"
            log_dim "Manual: place enterprise-attack.json in docs/"
        fi
    else
        log_info "Skipped — download later from Analysis → MITRE ATT&CK"
    fi
}

# ── Step 7 – External repositories ────────────────────────────────────────────

prompt_optional_repo() {
    local name="$1"
    local repo_key="$2"
    local target="$3"
    local size_note="$4"
    ...
    echo ""
    printf "  ${CYAN}?${NC}  ${BOLD}${name}${NC}\n"
    log_dim "${url} @ ${commit:0:12}"
    if [ -n "$size_note" ]; then
        printf "  ${YELLOW}⚠${NC}  ${size_note}\n"
    fi
    echo ""

    if prompt_yes_no "Download ${name} now?" "N"; then
...
download_repositories() {
    step "Optional CTI repositories"

    prompt_optional_repo \
        "DeepDarkCTI" \
        "deepdarkcti" \
        "$INSTALL_DIR/cti-platform/modules/deepdarkCTI-main" \
        ""

    prompt_optional_repo \
        "Ransomware Tool Matrix" \
        "ransomware_tool_matrix" \
        "$INSTALL_DIR/cti-platform/modules/Ransomware-Tool-Matrix-main" \
        ""
}
