...
# User-space compatible (no root required)
# ============================================================================
...
# Paths - detect script directory for user mode
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# Use environment variable or auto-detect based on script location
if [ -n "$CTI_INSTALL_DIR" ]; then
    INSTALL_DIR="$CTI_INSTALL_DIR"
else
    # Auto-detect: go up one level from scripts directory
...
if [ $? -eq 0 ] && [ -f "$CERT_FILE" ] && [ -f "$KEY_FILE" ]; then
    # Set permissions (user permissions, no root required)
    chmod 600 "$KEY_FILE"
    chmod 644 "$CERT_FILE"
    ...
    log_info "  cd $INSTALL_DIR && source venv/bin/activate"
    log_info "  cd cti-platform && python app.py"
else
